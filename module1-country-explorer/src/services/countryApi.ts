...
const FIELDS_DETAIL = 'name,cca3,capital,region,subregion,population,area,flags,languages,currencies,timezones,continents,borders,maps,landlocked,demonyms,car,coatOfArms';
...
export async function searchCountries(name: string): Promise<Country[]> {
  ...
  try {
    ...
    if (!Array.isArray(data)) throw new ApiError('Respuesta inesperada de la API');
    return data as Country[];
  } catch (error) {
    if (error instanceof ApiError) throw error;
    if (error instanceof TypeError) throw new ApiError('Error de conexión.', undefined, error);
    throw new ApiError('Error inesperado al buscar países', undefined, error);
  }
}

export async function getCountryByCode(code: string): Promise<Country | null> {
  const url = `${BASE_URL}/alpha/${encodeURIComponent(code)}?fields=${FIELDS_DETAIL}`;
  ...
  try {
    ...
    const data: unknown = await response.json();
    if (Array.isArray(data) && data.length > 0) return data[0] as Country;
    return data as Country;
  } catch (error) {
    if (error instanceof ApiError) throw error;
...
export async function getAllCountries(): Promise<Country[]> {
  ...
  try {
    const response = await fetch(url);
    if (!response.ok) throw new ApiError(`Error del servidor: ${response.status}`, response.status);
    const data: unknown = await response.json();
    if (!Array.isArray(data)) throw new ApiError('Respuesta inesperada de la API');
    return data as Country[];
  } catch (error) {
