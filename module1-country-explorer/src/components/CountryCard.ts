...
export function createCountryCard(
  ...
  card.innerHTML = `
    <div class="relative">
      ...
      <!-- Botón de favorito (PART 2) -->
      <button
        class="heart-btn absolute top-3 left-3 w-9 h-9 flex items-center justify-center rounded-full bg-slate-900/80 backdrop-blur-sm transition-all hover:scale-110"
        aria-label="${fav ? 'Quitar de favoritos' : 'Agregar a favoritos'}"
        data-cca3="${country.cca3}"
      >
        <span class="text-lg leading-none">${fav ? '♥' : '♡'}</span>
      </button>
    </div>
  ...
  // Manejador de click en botón de favorito (PART 2)
  const heartBtn = card.querySelector<HTMLButtonElement>('.heart-btn');
  heartBtn?.addEventListener('click', (event) => {
    event.stopPropagation(); // Evitar abrir el modal
    const nowFav = toggleFavorite(country.cca3);
    const span = heartBtn.querySelector('span');
    if (span) span.textContent = nowFav ? '♥' : '♡';
    heartBtn.setAttribute('aria-label', nowFav ? 'Quitar de favoritos' : 'Agregar a favoritos');
    // Disparar evento personalizado para que main.ts pueda re-filtrar
    card.dispatchEvent(new CustomEvent('favorite-toggled', { bubbles: true, detail: { cca3: country.cca3, isFav: nowFav } }));
  });
